...
import { Activity, AlertTriangle, CheckCircle, Server, FileText, FileSpreadsheet } from 'lucide-react';
...
const AnalyticsDashboard: React.FC = () => {
    ...
    const handleExportExcel = () => {
        ...
        const wsDataKPI = [
            ...
            ['Tasa de Uso %', data.utilizationRate.toFixed(1)],
            ['Incidentes Activos', data.activeIncidents],
            ['Incidentes Históricos', data.historicalIncidents],
            ['Software Top 1', softwareData.length > 0 ? softwareData[0].name : 'N/A'],
        ];
        const wsKPI = XLSX.utils.aoa_to_sheet(wsDataKPI);
        XLSX.utils.book_append_sheet(wb, wsKPI, 'KPIs Generales');

        // Sheet 2: Reservas por Día
        const wsDaily = XLSX.utils.json_to_sheet(dailyData.map(d => ({ Fecha: d.date, Reservas: d.count })));
        XLSX.utils.book_append_sheet(wb, wsDaily, 'Reservas Semanales');

        // Sheet 3: Software
        const wsSoftware = XLSX.utils.json_to_sheet(softwareData.map(d => ({ Software: d.name, Usos: d.count })));
        XLSX.utils.book_append_sheet(wb, wsSoftware, 'Top Software');

        XLSX.writeFile(wb, `Datos_LabManager_${new Date().getTime()}.xlsx`);
    };

    return (
        <div className="space-y-6 animate-fade-in pl-2">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <h1 className="text-2xl font-bold text-white">Tablero de Inteligencia de Negocios (BI)</h1>
                <div className="flex space-x-3">
                    <button
                        onClick={handleExportExcel}
                        className="flex items-center space-x-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm"
                    >
                    ...
                    <button
                        ...
                        className="flex items-center space-x-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm"
                    >
                        <FileText size={16} />
            ...
            <div ref={dashboardRef} className="space-y-6 bg-slate-50 dark:bg-transparent p-1 rounded-xl">
